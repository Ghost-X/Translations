	...
	for soup in soup_packets:
		if(engine == 'y'):
			...
			if(len(findsoup) > 0):
				...
				print("\nSonuc bulunamadı, komutları duzgun gırmeyı deneyın.")
				sys.exit()
			
			
		else:
			print("\nError[E2]: NGeçerli bir arama motoru parametresi degıl.")
			print("--help komutunu yardım almak için kullanabilirsin..")
			sys.exit()
...
def checkVulnList(url_list):
	...
	print(bcolors.OKBLUE + "INFO: URL'ler bu method ile deneniyor.  ' method" + bcolors.ENDC)
	vuln_list = []
	...
	for url in url_list:
		...
		try:
			os.system('clear')
			header()
			print(bcolors.OKBLUE + "INFO: Sabırlı olun.", str(len(url_list)), "urls" + bcolors.ENDC)
			print(bcolors.OKBLUE + "INFO: Bir sonraki siteye geçmek için CTRL+C yapabilirsiniz." + bcolors.ENDC)
...
def header():
	...
	print("#  Translator git's: https://github.com/Ghost-X/      #")
	print("#######################################################")
...
def loadingMsg():
	print(bcolors.OKBLUE + "LOADING: Program hazırlık aşamasında kahve içmeye başladı.(Espiri.)" + bcolors.ENDC)
	print(bcolors.OKBLUE + " Turkish Translation: Ghost-x" + bcolors.ENDC)
	print()
...
def displayResults(vuln_list):
	...
	print()
	if(len(vuln_list) == 0):
		print(bcolors.WARNING + "SQL injection açığı olan bir site bulunamadı." + bcolors.ENDC)
		sys.exit()
	else:
		print(bcolors.OKBLUE + "INFO: Aynı sitelerin sonuçları düzenleniyor." + bcolors.ENDC)
		...
		print(bcolors.FAIL + str(len(unique_vuln_list)) + " Muhtemelen açık bulunan bir site bulundu." + bcolors.ENDC)
...
def findAdmin(find_admin_url):
	...
	if(find_admin_url[:4] != "http"):
		print("Error: Linkinizi kontrol edin.\n")
		sys.exit()
	...
	if(os.path.exists("admin_list")):
		...
		print("Error: Dosya bulunamadı: admin_list\n")
		sys.exit()
...
Example: python3 sqldriveby.py --term=anasayfa.php?id= --engine=g --depth=5
Required Arguments:
	
	--term=     Bu Parametre dorkunuzu belirlemenize yarar Ornek: --term=index.php?id=
	
Optional Arguments:
	
	--engine=   Arama motoru seçmenize yarar. (y=Yahoo / b=Bing / g=Google)
	
	--depth=    Kaç sayfanın taranacağını bu dork ile belirtebilirsiniz.
	
Additional Features:

	--find-admin=    Admin panelini bulmak için kullanabileceğiniz bir parametre.
			 Ornek--find-admin="http://www.yourpage.com"
